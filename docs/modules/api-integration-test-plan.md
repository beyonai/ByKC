...
| FI1–FI11 | 文件导入显式 metadata、front matter 合并、自动创建父目录的请求元数据继承、zip 公共值、参数校验和事务回滚 | `test_import_metadata_merges_with_front_matter_and_can_disable_it`、`test_zip_import_applies_common_metadata_before_each_markdown_front_matter`、`test_import_metadata_failure_rolls_back_entry_storage_and_values` | 已写 |
...
| DM1–DM13 | 目录创建、更新、get/update、幂等、重命名/移动保留、move 新建目标目录 metadata、空目录/子树删除 | `test_directory_create_and_rename_share_entry_metadata_lifecycle`、`test_directory_metadata_update_operations_are_atomic`、`test_directory_metadata_follows_move_and_is_removed_with_subtree`、`test_move_metadata_applies_only_to_new_target_directories`、`test_empty_directory_metadata_disappears_from_every_entry_interface` | 已写 |
