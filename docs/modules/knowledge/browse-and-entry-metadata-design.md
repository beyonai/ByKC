...
`type` 的允许值为 `file` 和 `directory`。调用方根据响应项的 `type` 判断该命中项是文件还是目录，不能通过本接口的入参预先限定资源类型。为兼容现有响应，路径字段继续使用 `filePath`，但其语义扩展为“文件或目录的知识库内完整路径”。
