...
> 状态说明：Discovery/Enrich 已一次性切换到 [文件与目录同步变更统一事件通知设计](./resource-mutation-background-callback-design.md) 定义的 `KnowledgeEventPublisher`。请求为历史兼容保留已弃用的 `extraParams`/`extra_params`，仅接收且忽略；状态响应和事件不包含该字段。`extParams` 不受支持；现有数据库 `extra_params` 列仅作为历史表结构保留，新数据使用默认空值且运行时不读取。
...
`request_params` 继续保存 `maxEntities`、`topK`、`force` 等 worker 执行参数快照。历史兼容字段 `extraParams`/`extra_params` 是可选且已弃用的请求字段：输入值不做规范化，但在进入 `request_params`、batch/task、状态响应和 Callback 事件前丢弃。表中现有 `extra_params` 列为避免本次引入破坏性 schema 迁移而保留，新行使用默认 `{}` 且业务运行时忽略。
...
Discovery 和 Enrich 为历史兼容，仅保留已弃用的可选字段 `extraParams`/`extra_params`，服务不使用也不向下游传递。`extParams` 不在请求模型中；由于模型使用 `extra="forbid"`，传入时返回请求校验失败。
...
- 不可处理文件创建为 `skipped`；
- 零文件批次立即完成；
- Discovery/Enrich 请求传入已弃用的 `extraParams`/`extra_params` 时可正常接收但不持久、不进入事件；传入 `extParams` 时校验失败；
...
9. batch 不因文件失败而失败；
10. Discovery/Enrich 请求仅兼容已弃用且被忽略的 `extraParams`/`extra_params`，不接受 `extParams`；
