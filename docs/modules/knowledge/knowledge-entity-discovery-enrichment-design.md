...
2. 记录输入 checksum；
3. 生成归一化文本及原文位置映射；
4. 对齐原始实现构建稳定的长文框架：在 16K 预算内保留约 2/3 开头、1/3 结尾和标题地图，单次抽取、不分块合并。
5. 以“抽取协议 + 文档上下文 + 实体上限 + 实际模型配置指纹”的 SHA-256 作为内容寻址键，在同一运行时内复用最近 128 份归一化候选快照；模型名称、地址、温度或兼容参数变化时自动隔离旧快照。词表匹配不进入键且不进入 prompt，因此相同模型配置下 A/B 内容一致时复用同一候选集，词表仍只负责后续身份解析。
...
4. 唯一精确命中产生规范名和身份提示，不绕过文档显著性判定直接写入关系；
5. 同名多候选保留上下文，进入消歧；
6. 词面重叠但不等价时不自动写入别名。
...
LLM 必须返回一份由文档内容决定的完整显著实体集。AC 结果不进入生成提示词，只在 LLM 返回后的身份治理阶段用于规范名和身份解析；因此同一正文的模型输入不随词表状态变化。每个候选至少包含：
...
候选只是任务中间结果，不立即成为文档或词表条目。信息抽取使用确定性采样（`temperature=0`）；但该设置本身不替代重复文档一致性评测。
...
只有进入完整显著实体集的候选才执行上述身份治理并持久化 `MENTIONS`。AC 命中但未通过删除测试的偶发提及不建立关系。
...
- 错误合并率；
- 每篇候选数、持久化实体数和裁决数。
- 相同正文重复抽取的实体名集合 Jaccard 相似度不低于 `0.85`；
- 同一正文在空词表与首次抽取结果已进入词表两种状态下，实体名集合 Jaccard 相似度不低于 `0.85`；
