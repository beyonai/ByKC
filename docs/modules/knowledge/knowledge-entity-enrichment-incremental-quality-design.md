...
## 15. ClaimGroup 脱离模型上下文与分类型引用

2026-08-28 的样本复核发现，把完整 `EvidenceClaimGroup`、claimCoverage、citationPlan 和按组
展开的 edit hints 序列化进 user message，会占用本应留给旧文档和召回原文的上下文空间。
ClaimGroup 因此调整为纯服务端编排结构：它继续影响证据的稳定顺序、来源完整性审计和评测归档，
但不再出现在 system/user message，也不再要求模型返回 ClaimGroup ID。`ExistingClaimAnchor` 仍仅
在增量模式进入消息，因为它直接保护旧文档事实，不属于召回证据的重复描述。

引用策略同步按来源类型拆分：

- `originalDocument` 是写作原材料。正文默认自然吸收其事实，不使用“根据某文章”起句或段末
  孤立链接；每个实际进入上下文的原始来源必须在“参考资料”中形成一次真实 Markdown 链接。
- `knowledgeEntity` 是已有知识实体。只有表达真实启发、比较、依赖或继承时，才在对应事实句中
  自然链接，例如“该设计受到了 [GBrain](...) 的启发”；不能把它当作段末脚注，也不能仅放入
  参考资料暗示不存在的关系。

确定性审计新增来源类型规则：原始文档链接出现在正文会触发修复，缺少参考资料链接也会触发
修复；KnowledgeEntity 必须在参考资料章节之前的正文中出现。质量评审器使用相同口径，避免继续
用旧的“每个主张附近都要行内引用”规则误判新文档。

三个样本的 Evidence 字符完全不变，而 prompt 字符分别下降：GBrain 25,218 → 15,929
（-36.8%）、OpenAI Frontier 4,433 → 3,414（-23.0%）、OpenClaw 31,342 → 20,465
（-34.7%）。最终接受 OpenAI Frontier 和 OpenClaw 两个真实样本：二者首次成文均通过，
OpenClaw 增量重放也通过；正文硬引用和段末链接均为 0，OpenClaw 的四个原始来源在参考资料中
各保留一个真实链接。GBrain 作为多 Topic 压力样本仍有事实遗漏和章节重叠，后续需要服务端
FactCluster，但不能重新把内部聚类结果展开写入模型上下文。

## 16. 词法语义拟合红线收敛

2026-08-28 的后续审查确立核心红线：禁止用正则、关键词、前后缀、字符串包含或
文档特例拟合语义结果。本节约束优先于前文中的字面实体限界、方面密度、
`ExistingClaimAnchor`、冲突关键词和定向修复设计。

收敛后的正常 enrich 路径只有一次 LLM 生成：

1. 已确认关系来源只按文档结构和字符上限分片，不再按实体名、别名、Topic 或
   产品标题删减内容。
2. 语义召回保留 `Entity + Topic` 查询和检索系统原始分数，不再通过字面实体名
   判断、前缀裁剪或样本 profile 规则二次过滤。
3. Topic × Source 预算只使用证据来源类型、已确认关系、检索分数和稳定定位
   字段排序；删除关键词方面分类和数字密度加分。
4. 完整旧 Markdown 仍原样进入上下文，但暂不实施 patch，也不再生成或校验
   `ExistingClaimAnchor`/`oldClaimRetention`。旧引用继续通过授权 URL 的精确集合合并保留。
5. 冲突、事实保留、重复、主语归属和引用自然度由同一次 enrich 生成在输出前自检；
   程序不再用关键词或模糊相似度触发第二次改写。
6. 原 `claim coverage` 更名为 source traceability，只表示授权来源链接是否出现在
   规定位置，不声称已验证主张语义覆盖。

仍允许确定性的 Markdown 结构解析、权限与文件 ID 校验、时间水位、授权 URL
白名单、字符预算、位置区间合并和完全相同内容去重。这些操作不对内容的
实体归属、事实含义或冲突关系作程序裁决。Topic 批处理本轮不调整。

## 17. 语义来源与关系原文预算收敛

移除词法限界后，真实样本暴露出两个新的容量问题：单个 Topic 可能带回多个跨实体来源；
已确认关系来源若整篇分片进入上下文，会与同源语义命中一起迅速占满总证据预算。
本轮采用不判断文本语义的通用收敛规则：

1. 每个 Topic 最多保留检索分数最高的两个语义来源。已确认关系来源不参与淘汰，
   并优先占用来源名额；其余名额再按来源在该 Topic 下的最高检索分数补足。
2. 每个已确认关系来源的整篇兜底证据最多 8,000 字符，按文档结构片段在首尾及
   中间位置均匀取样，避免只从文章开头顺序截满。该额度与语义命中额度分开。
3. 同一来源的关系片段和语义片段在内容或位置重合时仍合并为一份证据，语义命中携带的
   Topic 和分数合入关系证据。
4. 语义片段的前置上限由“每文档 25 个”改为“每 Topic × Source 3 个”，避免 Topic
   检索顺序靠前的结果占满名额，使后置 Topic 在进入统一证据预算前已经没有候选。

首轮三个真实样本中，Evidence 字符数从上一版的 GBrain 28,293、OpenClaw 50,000、
OpenAI Frontier 16,239，分别下降到 16,289、24,208、8,229；OpenClaw 与 OpenAI Frontier
通过内容评审，GBrain 因旧的全局 25 片段上限遗漏后置“部署路径”而未通过。修正为
Topic × Source 独立片段上限后，GBrain Evidence 为 17,502 字符，coverage 从 3 恢复为 5，
groundedness/synthesis/citationQuality/maintainability 均为 4，并通过内容评审；生成耗时从
167.086 秒下降到 79.340 秒。结构化来源审计同时兼容“参考资料”和“资料参考”标题，
避免把合法文末来源误报为正文硬引用或来源缺失。

## 18. 生成后引用规范化

模型生成完成后增加不调用 LLM 的引用规范化步骤：

1. 对 originalDocument，只识别正文段落末尾的孤立来源链接。命中后从正文移除，
   并确保同一链接在文末“参考资料”中保留一次；正文中用于精确引语、主体评价或冲突归因的
   自然行内链接不处理。
2. 证据收集阶段为 knowledgeEntity EvidenceFragment 携带权威 `entityName` 和 `aliases`。
   如果正文出现这些名称但尚未链接对应实体，则自动为第一次自然出现补充实体链接。
3. 名称候选按字符长度降序匹配，避免短别名先占用长实体名；同一 KnowledgeEntity 最多
   自动补充一次链接，同一名称若映射到多个目标则跳过。
4. 已有 Markdown 链接、标题、代码块、行内代码、URL 和参考资料章节不参与自动补链。

该规则只处理已授权来源的引用位置和已解析实体身份，不参与证据保留、事实归属或语义冲突判断，
因此不扩展词法语义拟合的范围。
