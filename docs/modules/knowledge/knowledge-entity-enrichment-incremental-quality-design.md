...
证据完成限额和去重后，在调用 LLM 前确定性生成 `EvidenceClaimGroup`。分组键为
`Topic × Source`，未携带 Topic 的实体级证据归入“实体总览”。每组记录：

- 稳定的 `claimGroupId`；
- `sourceId`、来源文件和关联 `evidenceIds`；
...
旧 Markdown 仍全文、不截断地放入输入；旧引用继续由链接集合差分做确定性恢复。因此增量编辑
计划负责语义融合，引用恢复负责最后的不可丢失底线。
