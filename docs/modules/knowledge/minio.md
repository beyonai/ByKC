...
- 为读取接口提供原文件下载来源和 Markdown 内容来源
- 与本地缓存索引协同，降低重复下载成本

## 存储职责边界

MinIO 只负责对象内容本身，不承担知识模块结构化状态管理。
...
```text
kb/12/item/345/version/v2026-04-07/original
```

设计意图：

- 以 `knowledge_base_id` 作为逻辑租户边界
- 以 `knowledge_item_id` 作为稳定对象标识
...
```text
kb/12/item/345/version/v2026-04-07/markdown
```

若原始文件不是 Markdown，sidecar 仍作为独立 Markdown 对象存储，便于后续 chunk 读取与文本缓存处理。
...
- 逻辑路径重命名不会要求 MinIO 对象随之迁移

## 读取链路设计

读取接口存在两种主要模式：

- 原始文件读取：供 `downloadFile` 直接返回文件流
- Markdown 读取：供 `readFile` 优先走本地缓存，缓存未命中时从 MinIO 下载

### 原始文件读取

原始文件由版本记录中的 `bucket_name` 与 `object_key` 定位，服务端读取对象后直接输出文件流。

### Markdown 读取

Markdown 不直接返回对象 URL，而是优先下载到本地缓存目录后再按全文或行窗口读取。

原因如下：

- `readFile` 支持按行区间读取，需要服务端可控地切片
...
当前知识模块以数据库软删除为主，MinIO 对象删除应视为派生清理动作，不应反向驱动业务状态变更。也就是说，业务可见性先由数据库决定，对象存储清理由后台任务或后续治理策略跟进。
