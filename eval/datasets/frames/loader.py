...
def _parse_query(record: dict, index: int) -> EvalQuery:
    ...
    query_id = str(record.get("query_id", index))
    question = record.get("question", "")
    ground_truth = record.get("answer", "")

    metadata = {
        k: v for k, v in record.items() if k not in ("query_id", "question", "answer")
    }
...
def load_frames_queries() -> list[EvalQuery]:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"FRAMES queries file not found at {path}. "
            "Set FRAMES_QUERIES_PATH env var or run download first."
        )
