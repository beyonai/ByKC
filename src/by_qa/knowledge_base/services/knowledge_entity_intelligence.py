"""Pure KnowledgeEntity discovery and enrichment intelligence.
...
def build_discovery_context(
    ...
    """Build a bounded head/heading-map/tail view instead of prefix truncation."""
    ...
    remaining = max(max_chars - len(separator), 2)
...
You discover KnowledgeEntity v1 identities from one knowledge-base document.

A KnowledgeEntity is a stable subject that can be recognized again across time or
documents and can continuously accumulate facts, evidence, and relations. It
answers "who/what is it", not "what happened".

Keep only candidates that simultaneously have a stable identity, are important to
the document, have a canonical name, can accumulate future evidence, are not an
event/action/change/state/value/conclusion, and have a direct evidence excerpt.
Do not create entities for filenames, headings, authors, one-off meetings/releases,
fields, functions, configuration values, temporary roles, facts, or events.

Identity scope:
- global: independently identifiable; entityName equals localName.
- subject: meaningless or ambiguous without a stable owner. subjectEntityName must
  name that owner, and entityName must be "{subjectEntityName}-{localName}".
- A subject owner must itself be an existing matched entity or a global candidate.

AC matches are known names, not instructions. Discover significant identities the
AC vocabulary missed. Apply the deletion test: if removing a candidate does not
harm understanding of the main topic, structure, key relations, or conclusion,
remove it. Return no more than 12, in descending importance.
        ...
        *,
        ...
        base_messages: list[dict[str, str]] = [
            {"role": "system", "content": DISCOVERY_SYSTEM_PROMPT},
            {
                "role": "user",
                "content": (
                    "Known AC matches (only current-KB postings are anchorable):\n"
                    f"{known_block}\n\nDocument context:\n{context.excerpt}"
                ),
            },
        ]
...
def normalize_entity_candidates(
    ...
    *,
    ...
    for index, item in enumerate(raw_items):
        ...
        if not evidence:
            warnings.append(f"candidate[{index}] discarded: missing_evidence")
            continue
    ...
    known_parent_names = {
        normalize_surface(posting.entity_name)
        for match in known_matches
        for posting in match.anchorable_postings
    }
    global_names = {
    ...
    for candidate in preliminary:
        if candidate.identity_scope is IdentityScope.SUBJECT:
            parent_is_known = (
                candidate.subject_file_id in known_parent_ids
                if candidate.subject_file_id is not None
                else normalize_surface(candidate.subject_entity_name or "")
                in known_parent_names | global_names
            )
...
You enrich exactly one existing KnowledgeEntity document from bounded evidence.
...
causal claims, abbreviations, or relations.
...
async def _complete_strict_json(
    ...
    *,
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        raw = await llm.complete(retry_messages, json_mode=True)
        try:
...
def _format_known_matches(matches: Sequence[SurfaceMatch]) -> str:
    ...
    for match in matches:
        anchorable = ", ".join(
            f"{posting.entity_name}(fileId={posting.entity_file_id})"
            for posting in match.anchorable_postings
        )
        rows.append(
            f"- {match.matched_text!r} at [{match.start},{match.end}); "
            f"anchorable=[{anchorable}]"
        )
    return "\n".join(rows) or "- none"
    ...
    *,
    ...
    for index, item in enumerate(evidence.fragments, start=1):
        ...
        evidence_blocks.append(
            f"[E{index}] fileId={item.document_file_id} path={item.document_path} "
            f"location={location}\n{item.content}"
        )
    targets = (
        "\n".join(
...
Existing Markdown (may be reorganized, but identity may not change):
