...
def test_can_import_qa_instant_modules():
    instant_module = importlib.import_module("by_qa.qa.engines.instant")
    assert instant_module
    assert hasattr(instant_module, "InstantSearchState")
    assert hasattr(instant_module, "InstantQAState")

    assert importlib.import_module("by_qa.qa.engines.instant.state")

    instant_types = importlib.import_module("by_qa.qa.engines.instant.types")
